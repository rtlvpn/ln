...
function renderChart(candlestickData, heatmapData) {
    ...
    // Choose implementation based on UI selection
    const implementationType = document.getElementById('implementationType')?.value || 'p5';
    let predictionData;
    
    // Create loading indicator
    const chartElement = document.getElementById('chart');
        const loadingDiv = document.createElement('div');
        loadingDiv.id = 'tensorLoadingIndicator';
        loadingDiv.className = 'tensor-loading';
    
    // Remove any existing loading indicators
    const existingIndicator = document.getElementById('tensorLoadingIndicator');
    if (existingIndicator) existingIndicator.remove();
    
    // Switch between implementations
    if (implementationType === 'tensor' && typeof tf !== 'undefined') {
        // Show loading indicator during tensor processing
        loadingDiv.textContent = 'Computing with TensorFlow.js...';
        chartElement.appendChild(loadingDiv);
        
        // Process asynchronously
        calculatePricePredictionTensor(heatmapData, candlestickData, pathCount)
            .then(data => {
                // Remove loading indicator
                document.getElementById('tensorLoadingIndicator')?.remove();
                
                // Cache and proceed with rendering
                window.cachedPredictionData = data;
                predictionData = data;
                continueRendering();
            })
            .catch(error => {
                ...
                // Fallback to P5
                predictionData = calculatePricePredictionP5(heatmapData, candlestickData, pathCount);
                window.cachedPredictionData = predictionData;
                continueRendering();
            });
        
        return; // Early return for async processing
        } else {
        // Use P5.js implementation (synchronous)
        predictionData = calculatePricePredictionP5(heatmapData, candlestickData, pathCount);
        window.cachedPredictionData = predictionData;
        continueRendering();
    }
    
    // Helper function to continue rendering after prediction is available
